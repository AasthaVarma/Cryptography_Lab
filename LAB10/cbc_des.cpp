...
string first_xor(string key, string block)
{
	string x ="";
...
int main(){ 
	...
	{
		blocks[k] = pt.substr(i, 16);
		k++;
	}
	key="AABB09182736CCDD"; 
	xyz="AAAAAAAAAAAAAAAA";
	//Key Generation 
	...
	string cipher[4], text[4];
	string x;
	/*
	for(int i=0; i<4; i++)
	{
		//cout << i << ":" << xyz.length() << " " << blocks[i].length() << endl;
		x = first_xor(xyz, blocks[i]);
		cipher[i]= encrypt(x, rkb, rk); 
		
		//cout<<"Cipher Text: "<<cipher[i]<<endl; 
		reverse(rkb.begin(), rkb.end()); 
		reverse(rk.begin(), rk.end()); 
		
		x = encrypt(cipher[i], rkb, rk); 
		text[i] = first_xor(xyz, cipher[i]);
		
		xyz = cipher[i];
		//cout<<"Plain Text: "<<text[i]<<endl; 
	}*/
